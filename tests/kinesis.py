...
def test_invalid_payload_key():
    is_success, message = push_to_stream("Sample", "some-access-key", "secret-access-key", 1234, "1234")
    assert is_success == False

@mock_kinesis
def test_invalid_payload():
    with pytest.raises(NotAJSONError):
        push_to_stream("Sample", "some-access-key", "secret-access-key", "1234", set([1,2,3]))

@mock_kinesis
def test_invalid_streamname():
    is_success, message = push_to_stream("sample", "some-access-key", "secret-access-key", "1234", {"a":123,"b":456})
    assert is_success == False
...
def test_valid_streampush():
    ...
    is_success, message = push_to_stream("sample-stream", "some-access-key", "secret-access-key", "1234", {"a":123,"b":456})
